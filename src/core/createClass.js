...
define(['./Utils'], function (Utils) {
  ...
  function splitProto (object) {
    ...
    for (var i in object) {
      ...
      if (Utils.isObject(object[i])) {
        // TODO Error
        // object-throw 'Class initial properties cannot be an object or null. You should define them inside constructor (init) function.'
      }
  ...
  function isInstanceOf (Class) {
    if (!Utils.isSet(Class._id)) false // object-throw 'Parameter has to be a Class'
  ...
  function isImplementedBy (Interface) {
    if (!Utils.isArray(Interface)) false // object-throw 'Parameter has to be an Interface'
  ...
  function createInstance (Class, childInstances, base) {
    ...
    if (!base) {
      ...
      instance = function () { this.super._init.apply(this.super, arguments) }
    }
